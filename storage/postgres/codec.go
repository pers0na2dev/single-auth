	...
	if id, exists := normalized["id"]; exists {
		if id != nil {
			if configuration.idType == SerialID {
				...
				if err != nil {
					return encodedMutation{}, fmt.Errorf("postgres: encode %s.id: %w", model.canonical, err)
				}
				id = numericID
			} else {
				id = fmt.Sprint(id)
	...
	for _, field := range modelFields(model)[1:] {
		...
		if create {
			if (!supplied || (value == nil && field.attribute.IsRequired())) && field.attribute.DefaultValue != nil {
				...
				if err != nil {
					return encodedMutation{}, fmt.Errorf("postgres: default %s.%s: %w", model.canonical, field.canonical, err)
				}
				supplied = true
			}
			...
			if err != nil {
				return encodedMutation{}, fmt.Errorf("postgres: on-update %s.%s: %w", model.canonical, field.canonical, err)
			}
			supplied = true
		}
		if !supplied {
		...
		if err != nil {
			return encodedMutation{}, fmt.Errorf("postgres: encode %s.%s: %w", model.canonical, field.canonical, err)
		}
...
func normalizeSerialID(value any) (int64, error) {
	...
	case uint:
		if uint64(typed) > math.MaxInt32 {
			return 0, fmt.Errorf("serial ID %d exceeds PostgreSQL INTEGER", typed)
		}
		numeric = int64(typed)
	case uint8:
		numeric = int64(typed)
	...
	if numeric < math.MinInt32 || numeric > math.MaxInt32 {
		return 0, fmt.Errorf("serial ID %d exceeds PostgreSQL INTEGER", numeric)
	}
	return numeric, nil
}
...
func normalizeNumberForDriver(value any) (any, error) {
	...
	case uint:
		if uint64(number) > math.MaxInt64 {
			return nil, fmt.Errorf("number %d exceeds PostgreSQL BIGINT", number)
		}
		return int64(number), nil
	case uint8:
		return int64(number), nil
...
func decodeValue(configuration *config, field resolvedField, value any) (any, error) {
	...
	if field.canonical == "id" || (configuration.idType == SerialID && field.attribute.References != nil && field.attribute.References.Field == "id") {
		if value == nil {
			return nil, nil
		}
		return fmt.Sprint(value), nil
