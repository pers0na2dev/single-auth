...
Configure Spotify social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Spotify`. The provider ID is `spotify`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/spotify`.

> **Info: Provider console**
>
> Create an application in the Spotify developer dashboard and register the callback URL.

## Configuration

```go
spotifyProvider, err := providers.Spotify(providers.Options{
...
```go
authorizationURL, err := spotifyProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/spotify",
})

tokens, err := spotifyProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/spotify",
})

profile, err := spotifyProvider.GetUserInfo(ctx, tokens)
```
...
- The default scope requests the account email.
- PKCE is used during code exchange.
