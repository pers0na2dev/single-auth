...
func (p *plugin) register(ctx *engine.Context) (contract.Response, error) {
	...
	if session == nil && !p.options.AllowDynamicClientRegistration {
		return contract.Response{}, oauthError(
			contract.StatusUnauthorized, "invalid_token",
	...
	for _, redirectURI := range redirectURIs {
		if !isSafeURLScheme(redirectURI) {
			return contract.Response{}, validationError(
	...
	clientID := ""
	if p.options.GenerateClientID != nil {
		clientID = p.options.GenerateClientID()
	}
	if clientID == "" {
	...
	clientSecret := ""
	if p.options.GenerateClientSecret != nil {
		clientSecret = p.options.GenerateClientSecret()
	}
	if clientSecret == "" {
	...
	storedSecret, err := p.storeClientSecret(ctx.GoContext(), clientSecret)
	if err != nil {
		return contract.Response{}, internalError(err)
	}
	name, _, err := optionalBodyString(body, "client_name")
	if err != nil {
		return contract.Response{}, err
	}
	logo, logoExists, err := optionalBodyString(body, "logo_uri")
	if err != nil {
		return contract.Response{}, err
	}
	now := p.clock().UTC()
	record := storage.Record{
		"name": name, "clientId": clientID, "clientSecret": storedSecret,
		"redirectUrls": strings.Join(redirectURIs, ","), "type": "web",
		"authenticationScheme": authMethod, "disabled": false,
	...
	response := map[string]any{
		...
		"client_id_issued_at": now.Unix(), "client_secret_expires_at": 0,
...
func (p *plugin) getOAuthClient(ctx *engine.Context) (contract.Response, error) {
	if _, err := p.options.Runtime.ResolveSession(ctx, true); err != nil {
		return contract.Response{}, err
	}
	clientID, _ := ctx.Param("id")
	client, err := p.findClient(ctx, clientID)
	if err != nil {
		return contract.Response{}, err
	}
	if client == nil {
		return contract.Response{}, oauthError(contract.StatusNotFound, "not_found", "client not found")
	}
	result := map[string]any{"clientId": client.ClientID, "name": client.Name, "icon": nil}
	if client.Icon != nil && *client.Icon != "" {
		result["icon"] = *client.Icon
	}
