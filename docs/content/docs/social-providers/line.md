...
Configure LINE social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.LINE`. The provider ID is `line`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/line`.

> **Info: Provider console**
>
> Create a LINE Login channel and add the callback URL.

## Configuration

```go
lineProvider, err := providers.LINE(providers.Options{
...
| User-info endpoint | `https://api.line.me/oauth2/v2.1/userinfo` |
...
```go
authorizationURL, err := lineProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/line",
})

tokens, err := lineProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/line",
})

profile, err := lineProvider.GetUserInfo(ctx, tokens)
```
...
- Profile data is decoded from the ID token first and falls back to user info; decoding does not itself verify the token.
- The direct ID-token verifier calls LINE's verification endpoint.
- PKCE is used.
