...
Configure Notion social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Notion`. The provider ID is `notion`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/notion`.
...
```go
notionProvider, err := providers.Notion(providers.Options{
...
| User-info endpoint | `https://api.notion.com/v1/users/me` |
...
```go
authorizationURL, err := notionProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/notion",
})

tokens, err := notionProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/notion",
})

profile, err := notionProvider.GetUserInfo(ctx, tokens)
```
...
- Authorization requests identify the owner as a user.
- Authorization-code exchange uses HTTP Basic client authentication; refresh sends the client credentials in the POST body.
