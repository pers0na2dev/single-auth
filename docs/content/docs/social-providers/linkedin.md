...
Configure LinkedIn social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.LinkedIn`. The provider ID is `linkedin`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/linkedin`.

> **Info: Provider console**
>
> Create a LinkedIn application, request the Sign In with LinkedIn using OpenID Connect product, and add the callback URL.

## Configuration

```go
linkedinProvider, err := providers.LinkedIn(providers.Options{
...
```go
authorizationURL, err := linkedinProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/linkedin",
})

tokens, err := linkedinProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/linkedin",
})

profile, err := linkedinProvider.GetUserInfo(ctx, tokens)
```
...
- The provider requests profile, email, and openid by default.
- The authorization request carries LoginHint when supplied.
