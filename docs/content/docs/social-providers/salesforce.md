...
Configure Salesforce social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Salesforce`. The provider ID is `salesforce`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/salesforce`.

> **Info: Provider console**
>
> Create a Salesforce Connected App and add the callback URL.

## Configuration

```go
salesforceProvider, err := providers.Salesforce(providers.Options{
...
auth, err := singleauth.New(singleauth.Options{
    ...
    SocialProviders: map[string]*providers.Provider{
        salesforceProvider.ID: salesforceProvider,
    },
})
```
...
| User-info endpoint | `https://login.salesforce.com/services/oauth2/userinfo` |
...
```go
authorizationURL, err := salesforceProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/salesforce",
})

tokens, err := salesforceProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/salesforce",
})

profile, err := salesforceProvider.GetUserInfo(ctx, tokens)
```
...
- Use LoginURL for a My Domain host rather than rewriting individual endpoints.
