...
Configure Kakao social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Kakao`. The provider ID is `kakao`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/kakao`.

> **Info: Provider console**
>
> Create a Kakao application, enable Kakao Login, and register the callback URL.

## Configuration

```go
kakaoProvider, err := providers.Kakao(providers.Options{
...
auth, err := singleauth.New(singleauth.Options{
    ...
    SocialProviders: map[string]*providers.Provider{
        kakaoProvider.ID: kakaoProvider,
    },
})
```
...
```go
authorizationURL, err := kakaoProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/kakao",
})

tokens, err := kakaoProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/kakao",
})

profile, err := kakaoProvider.GetUserInfo(ctx, tokens)
```
...
- The provider maps Kakao account and profile nesting and marks email verified only when Kakao reports both validity and verification.
- The authorization request does not use PKCE.
