...
func validateReservedNames(schema storage.Schema) error {
	for canonical, table := range schema.Models {
		...
		if err := validateMSSQLIdentifier("model", physicalModel); err != nil {
			return err
		}
		...
		for fieldName, attribute := range table.Fields {
			...
			if err := validateMSSQLIdentifier("field", physical); err != nil {
				return err
			}
			if err := validateMSSQLIdentifier("presence field", presencePrefix+fieldName); err != nil {
				return err
			}
		}
	}
...
func validateMSSQLIdentifier(kind, value string) error {
	if strings.IndexByte(value, 0) >= 0 {
		return fmt.Errorf("%w: SQL Server %s name contains NUL", storage.ErrInvalidQuery, kind)
	}
	// SQL Server sysname identifiers are limited to 128 UTF-16 code units.
	if utf16Length(value) > 128 {
		return fmt.Errorf("%w: SQL Server %s name %q exceeds 128 UTF-16 code units", storage.ErrInvalidQuery, kind, value)
	}
	return nil
}
