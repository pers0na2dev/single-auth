...
Configure Atlassian social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Atlassian`. The provider ID is `atlassian`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/atlassian`.

> **Info: Provider console**
>
> Create an OAuth 2.0 (3LO) app in the Atlassian developer console and register the exact callback URL.

## Configuration

```go
atlassianProvider, err := providers.Atlassian(providers.Options{
...
auth, err := singleauth.New(singleauth.Options{
    ...
    SocialProviders: map[string]*providers.Provider{
        atlassianProvider.ID: atlassianProvider,
    },
})
```
...
| User-info endpoint | `https://api.atlassian.com/me` |
...
```go
authorizationURL, err := atlassianProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/atlassian",
})

tokens, err := atlassianProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/atlassian",
})

profile, err := atlassianProvider.GetUserInfo(ctx, tokens)
```
...
- The authorization request includes Atlassian's audience and offline-access requirements.
- Both client credentials and PKCE are required when the authorization URL is created.
