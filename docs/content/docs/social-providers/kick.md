...
Configure Kick social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Kick`. The provider ID is `kick`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/kick`.

> **Info: Provider console**
>
> Create a Kick application and register the callback URL.

## Configuration

```go
kickProvider, err := providers.Kick(providers.Options{
...
| User-info endpoint | `https://api.kick.com/public/v1/users` |
...
```go
authorizationURL, err := kickProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/kick",
})

tokens, err := kickProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/kick",
})

profile, err := kickProvider.GetUserInfo(ctx, tokens)
```
...
- The provider uses PKCE and normalizes Kick's nested user response.
