...
Configure Zoom social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Zoom`. The provider ID is `zoom`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/zoom`.

> **Info: Provider console**
>
> Create a Zoom OAuth application and add the callback URL to its allow list.

## Configuration

```go
zoomProvider, err := providers.Zoom(providers.Options{
...
| User-info endpoint | `https://api.zoom.us/v2/users/me` |
| Default scopes | `None by default` |
| Token client authentication | `post` |
...
| Field | Behavior |
| --- | --- |
| `PKCE` | Pointer boolean controlling PKCE. Nil means enabled, matching the default. |
...
```go
authorizationURL, err := zoomProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/zoom",
})

tokens, err := zoomProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/zoom",
})

profile, err := zoomProvider.GetUserInfo(ctx, tokens)
```
...
- Token exchange uses client credentials in the POST body.
- PKCE is enabled by default and can be explicitly disabled only when required by an existing Zoom app.
