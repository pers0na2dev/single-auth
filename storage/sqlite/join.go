...
func (e *executor) prepareJoins(base resolvedModel, requested map[string]storage.JoinOption, selectFields *[]string) ([]joinPlan, error) {
	...
	for _, name := range names {
		option := requested[name]
		joinedModel := name
		if option.Model != "" {
			joinedModel = option.Model
		}
		joined, err := resolveModel(e.config, joinedModel)
		if err != nil {
			return nil, err
		}
		plan, required, err := e.deriveJoin(base, joined, option)
		if err != nil {
			return nil, err
		}
		plan.outputKey = joined.canonical
		if option.Model != "" || option.RelationName != "" {
			plan.outputKey = name
		}
...
func (e *executor) deriveJoin(base, joined resolvedModel, option storage.JoinOption) (joinPlan, string, error) {
	...
	if len(keys) == 0 {
		if option.RelationName != "" {
			return joinPlan{}, "", fmt.Errorf(
				"%w: no foreign key named %q between %q and %q",
...
func (e *executor) joinRecord(ctx context.Context, base map[string]any, plan joinPlan) (any, error) {
	...
	predicate, args := buildRawEquality(toField, base[plan.config.From])
	fields := modelFields(plan.model)
	query := fmt.Sprintf(
		"SELECT %s FROM %s WHERE %s LIMIT ?",
		projection(fields),
		quoteIdentifier(plan.model.physical),
		predicate,
	)
