...
func (e *executor) joinRecord(ctx context.Context, base map[string]any, plan joinPlan) (any, error) {
	...
	limitPlaceholder := fmt.Sprintf("@p%d", len(args)+1)
	fields := modelFields(plan.model)
	query := fmt.Sprintf(
		"SELECT TOP (%s) %s FROM %s WHERE %s",
		limitPlaceholder, projection(fields), quoteIdentifier(plan.model.physical), predicate,
	)
