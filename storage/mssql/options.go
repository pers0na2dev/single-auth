...
const (
	...
	// SQL Server adapter generates them client-side.
	UUIDID IDType = "uuid"
	// SerialID uses INTEGER IDENTITY(1,1). Missing IDs are generated by SQL
	// Server and returned through OUTPUT inserted.
...
func normalizeOptions(options Options) (config, error) {
	...
	if err := schema.Validate(); err != nil {
		return config{}, fmt.Errorf("mssql: schema: %w", err)
	}
	...
	if defaultLimit < 0 {
		return config{}, fmt.Errorf("mssql: default limit must be non-negative")
	}
	...
	if idType != TextID && idType != UUIDID && idType != SerialID {
		return config{}, fmt.Errorf("mssql: unsupported ID type %q", idType)
	}
	...
	// These flags describe the native SQL Server representation. JSON/arrays are
	// stored as VARCHAR, dates are exchanged as ISO strings, and SMALLINT carries
	// booleans; codec.go performs all conversions.
...
func randomID(string) (any, error) {
	...
	if _, err := rand.Read(buffer); err != nil {
		return nil, fmt.Errorf("mssql: generate ID: %w", err)
	}
...
func randomUUID(string) (any, error) {
	buffer := make([]byte, 16)
	if _, err := rand.Read(buffer); err != nil {
		return nil, fmt.Errorf("mssql: generate UUID: %w", err)
	}
