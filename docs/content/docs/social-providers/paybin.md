...
Configure Paybin social sign-in in the native Go server.

Import `github.com/pers0na2dev/single-auth/protocol/providers` and construct the provider with `providers.Paybin`. The provider ID is `paybin`, so its default callback URL is `https://<your-auth-host>/api/auth/callback/paybin`.

> **Info: Provider console**
>
> Create a Paybin OAuth client and register the callback URL.

## Configuration

```go
paybinProvider, err := providers.Paybin(providers.Options{
    ClientID:     os.Getenv("PAYBIN_CLIENT_ID"),
    ClientSecret: os.Getenv("PAYBIN_CLIENT_SECRET"),
})
if err != nil {
    return err
}
...
auth, err := singleauth.New(singleauth.Options{
    ...
    SocialProviders: map[string]*providers.Provider{
        paybinProvider.ID: paybinProvider,
    },
})
```
...
| Property | Value |
| --- | --- |
| Authorization endpoint | `https://idp.paybin.io/oauth2/authorize` |
| Token endpoint | `https://idp.paybin.io/oauth2/token` |
...
| Field | Behavior |
| --- | --- |
| `Issuer` | Overrides the Paybin issuer/base URL when using a non-default deployment. |
...
```go
authorizationURL, err := paybinProvider.CreateAuthorizationURL(providers.AuthorizationInput{
    State:        state,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/paybin",
})

tokens, err := paybinProvider.ValidateAuthorizationCode(ctx, providers.CodeInput{
    Code:         code,
    CodeVerifier: verifier,
    RedirectURI:  "https://auth.example.com/api/auth/callback/paybin",
})

profile, err := paybinProvider.GetUserInfo(ctx, tokens)
```
...
- PKCE is used and the provider supports issuer-based endpoint selection.
- Profile data is decoded from the ID token, but this built-in does not expose a provider-specific ID-token verifier.
